...
7. you should be able to view the database in the dmt now, so you can get to work! build your models, investigate what you wish! hope you have a good time.
...
2. run the query and write the output table into the write-database using the dbt command `dbt run` (or be more specific with `dbt run -s [model_name]`)
...
4. refresh the database within the dmt. in dbeaver it is the 'invalidate/reconnect' button, which is a little plug-sign with a refresh arrow in the top-left corner.
   voila, you should be able to query the tables from within the dmt now.
